...
if __name__ == "__main__":
    ...
    while True:
        bef_pict = time.time()
        ret, img = capt.read()
        print "Capture   in " + str(time.time() - bef_pict)
        bef_det = time.time()
        tmp = (c.direction2center(img))
        print "Detection in " + str(time.time() - bef_det)
        count += 1
#         if(count >= 10):
#             time_taken_10 = time.clock() - start_10
#             print("Framerate = " + str(count/time_taken_10))
#             start_10 = time.clock()
#             count = 0
