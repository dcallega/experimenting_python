...
detector = compass(do_show_image=False)
for _ in range(NUMBER_OF_PICTURES):
    ret, img = capture.read()
    t_pos = [0,0,0]
    t_pos[0], t_pos[1], t_pos[2], image = detector.direction2center(img, True)
    cv2.imwrite("./img/20171213_" + str(time.time()) + ".jpg", image)
    x_action, y_action, z_action = 0,0,0
    if verbose:
        print("Called for action", [str(e) for e in t_pos])
    if abs(t_pos[0]) > X_CUTOFF:       #does it make sense to move?
        if t_pos[0] > 0:              #target on my left?
            x_action = WEST
        else:
            x_action = EAST
    else:pass #not worth of it
