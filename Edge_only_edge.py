...
class Image_receiver(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        self.name = "ImageReceiver"
        self.is_running = False
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.data = ''
        
    def start(self):
        self.socket.bind((TCP_IP_EDGE, TCP_PORT))
        self.socket.listen(1)
        self.conn, self.addr = self.socket.accept()
        
    def _image_decoding(self,image_string):
    ...
    def run(self):
        self.is_running = True
        while 1:
            while len(self.data) < len("Size" + DELIMITER + "0000000" + DELIMITER + "1512708150.42" + DELIMITER):
                self.data += self.conn.recv(BUFFER_SIZE)
                #print("data = " + data)
                #print(str(len(data)) + "?<?" + str(len("Size" + DELIMITER + "0000000" + DELIMITER)))
            elems = self.data.split(DELIMITER)
            self.data = DELIMITER.join(elems[3:])
            if(elems[0] == "Size"):
                payload_size = int(elems[1])
                time_stamp = elems[2]
            else:
                print(elems)
                raise ValueError("Bad formatting")
        #     print("Expecting payload of " + str(payload_size))
        #     print("Time is " + time_stamp)
        #     print("data only after header " + data)
            while len(self.data) < payload_size + len(DELIMITER):
                self.data += self.conn.recv(BUFFER_SIZE)
        #     print("data after data rcv " + data)
        #     print("Received " + str(len(data)) + " chars")
            elems = self.data.split(DELIMITER)
            image_string = elems[0]
            #print("Payload long " + str(len(image_string)))
            self.data = DELIMITER.join(elems[1:])
            #print("After reading data:" + data)
            image = self._image_decoding(image_string)
            condition_cap_det.acquire()
            if self.verbose:
                print 'Condition acquired'
            if(len(self.queue) > 4):
...
class ImageSender(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        self.name = "ImageSender"
        self.is_running = False
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        
    def start(self):
        self.socket.connect((TCP_IP_EDGE, TCP_PORT))
        ...
        while time.time()-time_started < TIMEOUT:
            ...
            while True:
                if self.queue:
                    ...
                    message = jpg_as_text + DELIMITER
                    everything = DELIMITER.join(["Size", str(len(message)),"{:.2f}".format(time.time()), message])
                    self.socket.send(everything)
                    break
                if self.verbose:
                    print 'condition wait by %s' % self.name
                condition_cap_det.wait()
